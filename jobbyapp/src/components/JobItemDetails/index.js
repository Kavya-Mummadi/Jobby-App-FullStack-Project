...
class JobItemDetails extends Component {
    ...
    componentDidMount() {
        // fetch the job for the first time
        this.getJobData()
    }

    componentDidUpdate(prevProps) {
        // fetch the job only if URL param changes
    ...
    applyForJob = async () => {
        ...
        console.log('Applying for job ID:', jobData.id) // <- add this
        if (!jobData.id) {
            alert('Job ID not available yet')
            return
        }

        const jwtToken = Cookies.get('jwt_token')
        console.log(jwtToken)
        const response = await fetch('https://jobby-app-fullstack-project.onrender.com/jobs/${id}', {
            method: 'POST',
            headers: {
    ...
    getJobData = async () => {
        ...
        if (response.ok === true) {
            ...
            this.setState({
                ...
                isApplied: data.job_details.is_applied
            })
        } else {
            ...
            <div className="job-details-view-container">
                <div className="job-item">
                    ...
                    <div className="button-container">
                        <button type="button" className="apply-button" onClick={this.applyForJob}
                            disabled={this.state.isApplied}
                        >
                            {this.state.isApplied ? 'Applied ✓' : 'Apply Now'}</button>
                    </div>
                </div>
